...
void pyosl::init_basic(py::module_& m) {
  ...
  py::enum_<osl::Player>(m, "Player", py::arithmetic())
    ...
    .def_property_readonly("sign", py::overload_cast<osl::Player>(osl::sign), "+1 for black or -1 for white")
    .def("to_csa", py::overload_cast<osl::Player>(&osl::to_csa))
    .def_property_readonly("win_result", &osl::win_result)
    .def_property_readonly("loss_result", &osl::loss_result)
    ;
  ...
  py::enum_<osl::GameVariant>(m, "GameVariant", py::arithmetic())
    .value("Hirate", osl::HIRATE, "ordinary game")
    .value("Shogi816K", osl::Shogi816K, "started at a randomized opening position")
    .value("Aozora", osl::Aozora, "game without pawns")
    .export_values();
  ...
  py::class_<osl::MiniRecord>(m, "MiniRecord", py::dynamic_attr(), "a game record\n\n"
    ...
    .def_readonly("history", &osl::MiniRecord::history, "list of :py:class:`HashStatus` s")
  ...
  py::class_<osl::BasicHash>(m, "BasicHash", py::dynamic_attr(), "hash code for a state")
    ...
    .def(py::self != py::self)
    ;
  py::class_<osl::HashStatus>(m, "HashStatus", "redundant hash status stored in MiniRecord")
    .def("basic", &osl::HashStatus::basic)
    ;
  typedef osl::OpeningTree tree_t;
  auto tree_type = py::class_<tree_t>(m, "OpeningTree", py::dynamic_attr())
    .def(py::init<>())
    .def("__contains__", [](tree_t& t, std::pair<uint64_t, uint32_t> key) { return t.contains(key); })
    .def("__getitem__", [](tree_t& t, std::pair<uint64_t, uint32_t> key) { return t.read(key); })
    .def("size", &tree_t::size)
    .def("root_count", &tree_t::root_count, "visit count for the initial state")
    .def_static("load_binary", [](std::string filename, int threshold, bool modify){
      return *tree_t::load_binary(filename, threshold); },
      "filename"_a, "threshold"_a=1, "modify"_a=true, "load from file")
    .def("export_all", &tree_t::export_all)
    .def("save_binary", &tree_t::save_binary, "filename"_a, "save to file")
    .def("prune", &tree_t::prune, "threshold"_a, "filename"_a, "prune entries by visit counts")
    .def("retrieve_children", &tree_t::retrieve_children, "inspect moves from a given state")
    .def("mutable_clone", &tree_t::mutable_clone, "make an editable clone")
    .def("mutable_view", &tree_t::mutable_view, "try to get an editable view")
    ;
  typedef osl::OpeningTreeEditable mutable_tree_t;
  py::class_<mutable_tree_t, tree_t>(m, "OpeningTreeEditable", py::dynamic_attr())
    .def(py::init<>())
    .def("add", &mutable_tree_t::add, "record"_a, "add a record")
    .def("add_all", &mutable_tree_t::add_all, "records"_a, "add all records")
    .def("add_subrecords", &mutable_tree_t::add_subrecords, "subrecords"_a, "add all subrecords")
    .def("decay_all", &mutable_tree_t::decay_all, "coef"_a=.99, "decay all counts")
    .def("dfs", &mutable_tree_t::dfs, "threshold"_a=128, "address values by dfs")
    .def_static("from_record_set", &mutable_tree_t::from_record_set, "data"_a, "minimum_count"_a)
    .def_static("restore_from", &mutable_tree_t::restore_from,
                "vectors"_a,
                "restore from data generated by :py:meth:`export_all`")
    .def("__getitem__", &mutable_tree_t::operator[])
    ;
  ...
  py::class_<osl::OpeningTree::Node>(tree_type, "Node", py::dynamic_attr())
    ...
    .def_readonly("depth", &osl::OpeningTree::Node::depth)
    .def_readonly("age", &osl::OpeningTree::Node::age)
    .def_readonly("black_value_backup", &osl::OpeningTree::Node::black_value_backup)
    ...
    .def("black_advantage", &osl::OpeningTree::Node::black_advantage,
         "prior"_a=.001)
    .def("black_advantage_tree", &osl::OpeningTree::Node::black_advantage_tree,
         "prior"_a=.001)
  ...
  m.def("kif_file",
        ...
        "filepath"_a,
        "load a game record");
  m.def("turn", [](osl::BasicHash code) { return osl::turn(code); }, "side to move");
  ...
  m.attr("hirate_816k_id") = &osl::hirate_816k_id;
