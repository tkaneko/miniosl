...
void pyosl::init_state_np(py::module_& m) {
  ...
  py::class_<base_t>(m, "BaseState", py::dynamic_attr(),
    ...
    .def_property_readonly("turn", &base_t::turn, "player to move")
    .def_property_readonly("oturn", [](const base_t& s){ return osl::alt(s.turn()); }, "opponent side to move")
    ...
    .def("shogi816k_id", &base_t::shogi816kID, "id of Shogi816K if match")
    .def("guess_variant", &base_t::guess_variant, "game variant based on active pieces")
    .def("king_active", &base_t::king_active, "color"_a, "return color's king is active")
    .def("__repr__",
         [](const base_t &s) {
  ...
  py::class_<state_t, base_t>(m, "State", py::dynamic_attr(),
    ...
    .def("find_threatmate_1ply", &state_t::findThreatmate1ply, "try to find a threatmate move")
  ...
  m.def("shogi816k",
        ...
        "id"_a, "make a state");
  m.def("aozora",
        [](){ return osl::EffectState(osl::BaseState(osl::Aozora)); },
        "make an initial state for Aozora shogi");

  // functions depending on np
  ...
  m.def("collate_features",
        ...
        ":param decay_power: increase to sample opening positions in lower frequency (optional)"
        );
  ...
  auto f = [&](int l, int r, TID tid) {
    for (int i=l; i<r; ++i) {
      ...
      record.sample_feature_labels_to(i, iptr, pptr, vptr, aptr, i2ptr, lmptr, sidptr,
                                      decay_power.value_or(SubRecord::default_decay), tid);
    }
  };
