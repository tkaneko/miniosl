...
def test_ptype():
    ...
    assert miniosl.king.to_csa() == 'OU'
    assert miniosl.pawn.to_csa() == 'FU'
    assert int(miniosl.empty) == 0
    assert int(miniosl.edge) == 1
    assert miniosl.pawn.to_ja() == '歩'
...
def test_move():
    ...
    assert m3122.to_ja(board) == '☖２二銀'
    assert m3122.to_ja(board, m8822.dst) == '☖同銀'
