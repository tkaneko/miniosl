...
def save_sfen(record_seq, path=''):
    ...
    logging.info(f'declare {declare} ({declare/len(record_seq)*100:.2f})%')
    return counts
...
def selfplay_array(cfg,
                   nn_for_array: miniosl.inference.InferenceForGameArray,
                   player_a: miniosl.PlayerArray,
                   player_b: miniosl.PlayerArray,
                   output: str,
                   csv_writer   # difficult to annotate
                   ):
    logging.info(f'{player_a.name()} v.s. {player_b.name()},'
                 + f' {cfg.n_games} games')
    mgrs = miniosl.GameArray(cfg.parallel, player_a, player_b, nn_for_array,
                             cfg.ignore_draw)
